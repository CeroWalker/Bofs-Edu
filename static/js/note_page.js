document.addEventListener("DOMContentLoaded", () => {
    const noteClassSelect = document.getElementById("note-classSelect");
    const noteLessonSelect = document.getElementById("note-lessonSelect");
    const noteTopicSelect = document.getElementById("note-topicSelect");
    ...
    // Sınıf seçimi yapıldığında ders seçeneğini güncelle
    noteClassSelect.addEventListener("change", () => {
        const classSelected = noteClassSelect.value;
        noteLessonSelect.innerHTML = '<option selected>Seçiniz...</option>';
        noteTopicSelect.innerHTML = '<option selected>Seçiniz...</option>';
        ...
        if (subjects[classSelected]) {
            Object.keys(subjects[classSelected]).forEach(subject => {
                ...
                noteLessonSelect.appendChild(option);
            });
        }
    });

    // Ders seçimi yapıldığında konuları güncelle
    noteLessonSelect.addEventListener("change", () => {
        const classSelected = noteClassSelect.value;
        const lessonSelected = noteLessonSelect.value;
        noteTopicSelect.innerHTML = '<option selected>Seçiniz...</option>';
